...
1. 总体统计查询
2. 单个示例查询
3. [新增] 【生成 type_features 】Type_ID 特征解析并导出为 JSONL
"""
...
def query_sample_city(conn, city_name="北京", year=2014, limit=500):
    """示例查询：获取指定城市指定年份的不同Type数据"""
    print(f"\n=== 🔍 示例查询: {year}年来源城市包含 '{city_name}' (前{limit}条) ===")
    start_time = time.time()

    # 参数化查询，显示Type、总数、目标城市和概率
    sql = """
    SELECT Year, Type_ID, From_City, Total_Count, Stay_Prob,
           To_Top1, To_Top1_Prob, To_Top2, To_Top2_Prob, To_Top3, To_Top3_Prob
    FROM migration_data
    WHERE From_City LIKE ? AND Year = ?
    ORDER BY Total_Count DESC
    LIMIT ?
    """

    # DuckDB 可以直接返回 Pandas DataFrame，打印非常美观
    df = conn.execute(sql, [f'%{city_name}%', year, limit]).df()
