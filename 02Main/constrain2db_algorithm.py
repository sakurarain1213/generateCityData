...
def get_cached_base_probs(city_code: str, year: int):
    """缓存人口基准权重."""
    return tuple(PopulationSynthesizer.generate_weights(city_code, year))


@lru_cache(maxsize=None)
def get_cached_outflow_probs(city_code: str, year: int):
    """缓存流出概率基线."""
    return tuple(
        MigrationEngine.calculate_outflow_prob(seg, city_code, year)
...
def process_row(row_data_str: str):
    ...
    # 2. 生成1200个细分人群的基准人口
    base_probs = np.array(get_cached_base_probs(from_city_id, year))
    ...
    # 3. 计算流出人口
    # 计算每组的流出倾向
    outflow_probs = np.array(get_cached_outflow_probs(from_city_id, year))
...
def worker_process_line(args):
    """并行处理单行数据."""
    idx, line_str, separator = args
    line_tab = line_str if separator == '\t' else line_str.replace(',', '\t')
    try:
        row_results, _ = process_row(line_tab)
        return idx, row_results, None
    except Exception as exc:  # pragma: no cover - 防御性日志
        return idx, [], str(exc)


def writer_worker(queue, db_path, table_name):
    """独立写库进程: 顺序消费合成结果并写入 DuckDB."""
    try:
        con = duckdb.connect(db_path)
    except Exception as exc:
        print(f"[ERROR] 子进程无法连接 DuckDB: {exc}")
        return
    try:
        while True:
            payload = queue.get()
            ...
            if not payload:
                continue
            try:
                write_results_batch(con, payload, table_name)
            except Exception as exc:
                print(f"[ERROR] 子进程写入失败: {exc}")
    finally:
        con.close()
...
def write_results_batch(con, results, table_name):
    """将结果批量写入 DuckDB (复用连接)."""
    if not results:
        return
...
def save_to_duckdb(results, db_path="output.db", table_name="migration_data"):
    ...
    try:
        con = duckdb.connect(db_path)
        try:
            write_results_batch(con, results, table_name)
        finally:
            con.close()
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
        ...
        for idx, line_str in enumerate(data_lines):
            ...
            try:
                ...
                tasks.append((idx, line_str, separator))
            except Exception as e:
                skipped_count += 1
                line_no = idx + start_idx
                print(f"行 {line_no} 数据异常: {e} -> 内容: {line_str[:50]}...")

        if not tasks:
            print("无待处理行，直接结束。")
            print(f"\n[完成] 全表处理结束。总共跳过 {skipped_count} 行 (已存在或异常)，新处理 0 行。")
        else:
            worker_count = min(len(tasks), max(1, mp.cpu_count() - 1))
            print(f"计划启用 {worker_count} 个并行进程进行合成。")
            ctx = mp.get_context("spawn")
            queue = ctx.Queue(maxsize=worker_count * 4)
            writer = ctx.Process(target=writer_worker, args=(queue, DB_FILE, TABLE_NAME))
            writer.start()
            error_count = 0

            try:
                with ctx.Pool(processes=worker_count) as pool, tqdm(total=len(tasks), desc="合成进度", unit="行") as pbar:
                    for idx, row_results, err in pool.imap_unordered(worker_process_line, tasks):
                        pbar.update(1)
                        if err:
                            line_no = idx + start_idx
                            print(f"行 {line_no} 数据异常: {err}")
                            error_count += 1
                            continue

                        if row_results:
                            queue.put(row_results)
                            processed_count += 1
            ...
            print(f"\n[完成] 全表处理结束。总共跳过 {skipped_count} 行 (已存在或异常)，新处理 {processed_count} 行，失败 {error_count} 行。")