...
DIMENSIONS = {
    ...
    'D4': {
        'name': '行业赛道',
        'values': ['Agri', 'Mfg', 'Service', 'Wht'],  # 农业、蓝领制造、传统服务、现代专业服务
        'labels': ['农业', '蓝领制造', '传统服务', '现代专业服务']
    },
    'D5': {
        'name': '相对收入',
...
# 长尾分布控制阈值
MIN_TYPE_COUNT = 1  # 最小Type样本数阈值（0表示只过滤count=0的Type）
# 说明：降低阈值以保留更多Type组合，增加数据多样性
# - 设置为3时：约1万有效Type（2.6%）
# - 设置为0时：约10万有效Type（25%）
# - 总人口保持不变，只是分散到更多Type中

# 人口基数配置
TOTAL_POPULATION_BASE = 1000_0000  # 总人口基数（可配置，单位：人）
# 说明：这是所有Type在所有城市的年总人口上限
# 实际分配逻辑：
#   1. 每个城市根据权重（GDP × 城市等级）分配人口配额
#   2. 城市内的Type根据幂律分布和经济学特征分配人口
#   3. 月人口 = 年人口 / 12 × 季节因子
#
# 示例：500万人口基数下
#   - 北京配额 ≈ 500万 × 4.4 / 34.9 ≈ 63万人
#   - 上海配额 ≈ 500万 × 4.7 / 34.9 ≈ 67万人
#   - 其他城市按权重递减
...
# 幂律分布参数
POWER_LAW_ALPHA = 1.8  # 从2.5降低到1.8，更符合人口特征分布（原2.5太极端）
